...
export async function GET() {
  ...
  const config = {
    ...
    const [resp1, resp2] = await Promise.all([ axios.get(`${process.env.FABRIC_API}/organization/all`, config), axios.get(`${process.env.FABRIC_API}/client/connections`, config)])
    ...
    connectedOrg.map((org: ConnectedOrganization) => {
      connectedOrgMap.set(org.organizationEmail, org)
    });
